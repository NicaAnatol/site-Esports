...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $team1_image = $_FILES['team1_image']['name'];
    $team1_score = $_POST['team1_score'];
    $team2_name = $_POST['team2_name'];
    $team2_image = $_FILES['team2_image']['name'];
    $team2_score = $_POST['team2_score'];
    $youtube_link = $_POST['youtube_link'];
    $twitch_link = $_POST['twitch_link'];

    if ($_FILES['team1_image']['error'] === UPLOAD_ERR_OK && $_FILES['team2_image']['error'] === UPLOAD_ERR_OK) {
        $target_dir = "images1/";
        if (!is_dir($target_dir)) {
            mkdir($target_dir, 0777, true);
        }
        $team1_image_path = $target_dir . basename($team1_image);
        $team2_image_path = $target_dir . basename($team2_image);

        move_uploaded_file($_FILES['team1_image']['tmp_name'], $team1_image_path);
        move_uploaded_file($_FILES['team2_image']['tmp_name'], $team2_image_path);

        $stmt = $conn->prepare("INSERT INTO league_data (user_id, league_name, categorie, team1_name, team1_image, team1_score, team2_name, team2_image, team2_score, youtube_link, twitch_link) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssississ", $_SESSION['user_id'], $league_name, $categorie, $team1_name, $team1_image_path, $team1_score, $team2_name, $team2_image_path, $team2_score, $youtube_link, $twitch_link);

        if ($stmt->execute()) {
            $_SESSION['result'] = 'success';
            $_SESSION['team1_image'] = $team1_image_path;
            $_SESSION['team2_image'] = $team2_image_path;
        } else {
...
<div class="create-form">
        <div class="border">
            <form action="profile.php" method="post" enctype="multipart/form-data">
                <h1>Profile</h1>
                <label for="league_name">Name legue:</label>
                ...
                <label for="team1_name">Name team 1:</label>
                <input type="text" id="team1_name" name="team1_name" required>

                <label for="team1_image">Image team 1:</label>
                <input type="file" id="team1_image" name="team1_image" accept="image/*" required>

                <label for="team1_score">
Score team 1:</label>
                <input type="number" id="team1_score" name="team1_score" required>

                <label for="team2_name">Name team 2:</label>
                <input type="text" id="team2_name" name="team2_name" required>

                <label for="team2_image">Image team 2:</label>
                <input type="file" id="team2_image" name="team2_image" accept="image/*" required>

                <label for="team2_score">Score team 2:</label>
                ...
                <label for="twitch_link">Link Twitch:</label>
                <input type="url" id="twitch_link" name="twitch_link" required>

                <button type="submit" class="button-style1">Submit</button>
                <?php if (isset($_SESSION['result'])): ?>
                    <div class="<?php echo $_SESSION['result'] == 'success' ? 'success' : 'error'; ?>">
                        <?php 
                            echo $_SESSION['result'] == 'success' ? 'Data submitted successfully!' : ($_SESSION['result'] == 'file_error' ? 'Error uploading files.' : 'Error submitting data.');
            ...
            <form action="profile.php" method="get">
                ...
                <button type="submit" class="button-style1">Logout</button>
            </form>
        </div>
    </div>
